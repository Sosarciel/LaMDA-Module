...
describe("Dialog-Domain DialogHelper 测试", () => {
    ...
    /**验证 getHistMessageList 能正确遍历消息链并返回完整结构
    ...
    /**验证 getCurrMessageList 返回完整的上下文组装结果
    ...
    /**验证 getCurrMessageList 的 maxCount 仅约束 hist-graph 部分
    ...
    /**验证 getCurrMessageList 的 onIntercept 截断后 memory 仍然存在
